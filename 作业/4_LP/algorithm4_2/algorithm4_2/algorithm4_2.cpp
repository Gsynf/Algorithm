// algorithm4_2.cpp : 定义控制台应用程序的入口点。
//
#include "stdafx.h"
#include<iostream>
#include<algorithm>
...
int main()
{
	...
	for (int i = 0; i < 10; i++)
	{
		if (i % 2 == 0)
			//左区间
		{
			b_matrix[i] = -st[i];             //-x1 <= -s1
			co_matrix[i][i / 2] = -1;
		}
		else
		{
			b_matrix[i] = st[i];             //x1 <= t1
			co_matrix[i][i / 2] = 1;
		}
	}
	co_matrix[10][0] = 1, co_matrix[10][1] = -1, co_matrix[10][5] = 1, b_matrix[10] = 0;       //x2-x1 >=d  ------>     x1-x2+d<=0
